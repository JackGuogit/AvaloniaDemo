...
using DeepseekConsoleApp.Models;

namespace DeepseekConsoleApp.Services
{
    public class DeepseekApiClient
    {
        ...
        public async Task<ChatCompletionResponse?> CreateChatCompletionAsync(ChatCompletionRequest request)
        {
            try
            {
                ...
                Console.WriteLine($"发送请求到Deepseek API: {json}");
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    Console.WriteLine($"API请求失败: {response.StatusCode} - {errorContent}");
                    return null;
                }

                var responseJson = await response.Content.ReadAsStringAsync();
                Console.WriteLine($"收到API响应: {responseJson}");
